...
export default function QuestionCard({
  ...
  return (
    <div className="w-full max-w-5xl mx-autorounded-3xl transition-all duration-500">
      {/* السؤال */}
      <h2 className="text-center text-2xl sm:text-3xl lg:text-4xl font-extrabold mb-6 sm:mb-8 leading-relaxed tracking-wide bg-gradient-to-r from-yellow-400 via-yellow-300 to-yellow-500 bg-clip-text text-transparent">
        {question.text}
      </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {question.options.map((opt, index) => {
          ...
          const baseClasses =
            "relative p-5 rounded-3xl text-lg font-semibold flex items-center justify-between text-right min-h-[70px] sm:min-h-[90px] transition-transform duration-300 transform focus:outline-none focus:ring-4 focus:ring-yellow-400 disabled:cursor-not-allowed disabled:opacity-60 shadow-lg";
          let bgClasses =
          ...
          let hoverClasses =
            "hover:scale-105  hover:bg-gradient-to-br hover:from-yellow-600 hover:via-yellow-500 hover:to-yellow-600";
          ...
          return (
            <button
              ...
              className={`${baseClasses} ${bgClasses} ${!isCurrentQuestionAnswered ? hoverClasses : ""}`}
            ...
            >
              ...
              {/* نص الخيار */}
              <span className="mr-12 text-base sm:text-lg truncate">{opt.text}</span>
